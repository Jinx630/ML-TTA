...
from data.coco2014 import COCO2014
from data.voc2007 import VOC2007
from data.nuswide import NUSWIDE
from data.object365 import OBJECT365
...
ID_to_DIRNAME={
    ...
    'nuswide': 'NUSWIDE',
    'object365': 'object365',
    'voc2012': 'VOC2012/PascalVOC2012/VOC2012_train_val',
    'lvis': 'COCO'
}
...
def build_dataset(set_id, transform, data_root, mode='test', n_shot=None, split="all", bongard_anno=False):
    if set_id == 'I':
        ...
        if set_id in ['coco2014', 'coco2017']:
            testset = COCO2014(set_id, os.path.join(data_root, ID_to_DIRNAME[set_id.lower()]), transform)
        elif set_id in ['voc2007', 'voc2012']:
            testset = VOC2007(set_id, os.path.join(data_root, ID_to_DIRNAME[set_id.lower()]), transform)
        elif set_id == 'nuswide':
            testset = NUSWIDE(set_id, os.path.join(data_root, ID_to_DIRNAME[set_id.lower()]), transform)
        elif set_id == 'object365':
            testset = OBJECT365(set_id, os.path.join(data_root, ID_to_DIRNAME[set_id.lower()]), transform)
        # elif set_id == 'lvis':
        #     testset = LVIS(set_id, os.path.join(data_root, ID_to_DIRNAME[set_id.lower()]), transform)
        elif mode == 'train' and n_shot:
